...
static bool __processIniLine(
  ...
  } else if (__strQuery(COMMENT_PREFIX, strlen(COMMENT_PREFIX), line[0])) {
    /*
     * ignore comment line
     */
  } else if (*hasSection) {
    bool ret =
        __parseSectionLine(line, region.len, cb, usr, ini.allowInlineComment);
    if (!ret) {
      return false;
    }
  } else {
 ...
 * or NULL if error can not ignored
 */
static const char *__processIniRegion(
  ...
  while (cursor < bufEnd) {
    ...
    if (!__processIniLine(
            ini, __region(line, lineEnd - line), cb, usr, hasSection)) {
      if (!ini.allowError) {
        return NULL;
      }
    }
...
static bool __iniParseFromReader(
  ...
  for (;;) {
    ...
    int len = off + ret;
    const char *next =
        __processIniRegion(ini, __region(ini.buf, len), cb, usr, &hasSection);
    if (!next) {
      return false;
    }
   ...
   */
  if (off) {
    if (!__processIniLine(ini, __region(ini.buf, off), cb, usr, &hasSection)) {
