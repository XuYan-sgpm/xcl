...
int ArrayBuffer::position(int idx) const {
...
char *ArrayBuffer::at(int idx) const { return buf + position(idx); }

int ArrayBuffer::next(int idx, int step) const { return position(idx + step); }

int ArrayBuffer::capacity() const { return cap; }

int ArrayBuffer::size() const {
  int sub = finish - start;
  return sub >= 0 ? sub : cap - start + finish;
}

bool ArrayBuffer::empty() const { return finish == start; }

bool ArrayBuffer::add(int idx, char ch) {
  if (reserve(idx, 1)) {
    *at(idx) = ch;
    return true;
  }
...
bool ArrayBuffer::reserve(int idx, int n) {
  if (checkAdd(idx, n)) {
    int s = size();
    if ((idx << 1) < s) {
      move(-n, 0, idx);
...
void ArrayBuffer::move(int dstIdx, int srcIdx, int n) {
...
void ArrayBuffer::fill(int dstIdx, int n, char ch) {
  if (n > 0) {
...
void ArrayBuffer::copy(int dstIdx, char *src, int n) {
  if (n > 0) {
    int i = position(dstIdx);
    int len;
    while (n > 0) {
...
bool ArrayBuffer::add(int idx, int n, char ch) {
  if (reserve(idx, n)) {
    fill(idx, n, ch);
    return true;
  }
...
bool ArrayBuffer::add(int idx, char *src, int n) {
  if (reserve(idx, n)) {
    copy(idx, src, n);
    return true;
  }
...
bool ArrayBuffer::add(int idx, const Buffer &buffer, int offset, int n) {
  if (&buffer == this) {
    if (idx == offset) {
      return true;
    }
...
bool ArrayBuffer::del(int fromIdx, int n) {
...
void ArrayBuffer::wrap(char *src, int n) {
  buf = src;
  cap = n;
  start = 0;
  finish = start + n;
...
bool ArrayBuffer::reserve(int n) { return checkAdd(size(), n - size()); }
