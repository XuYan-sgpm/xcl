...
void *
SysAllocator::Allocate(unsigned int bytes) {
  return malloc(Align(bytes, 8));
}

void
SysAllocator::Deallocate(void *p, unsigned bytes) {
  free(p);
}

void *
SysAllocator::Reallocate(void *p, unsigned int oldBytes,
						 unsigned int newBytes) {
  unsigned newRealBytes = Align(newBytes, 8);
  if (Align(oldBytes, 8) == newRealBytes) { return p; }
  return realloc(p, newRealBytes);
}

SysAllocator &
SysAllocator::instance() {
  if (!globalInstance) { globalInstance = new SysAllocator(); }
