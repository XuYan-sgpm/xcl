...
namespace xcl {
void
Pipeline::Add(Functional *functional) {
  auto node = new Node();
...
Object *
Pipeline::Fire(bool reverse) {
  return Fire(nullptr, reverse);
}
Pipeline::~Pipeline() {
...
void
Pipeline::AddFirst(Functional *functional) {
  auto node = new Node();
  node->functional = functional;
  node->next = first_;
  first_ = node;
...
void
Pipeline::Add(const Pipeline &pipeline) {
...
void
Pipeline::AddFirst(const Pipeline &pipeline) {
  Pipeline tmp_pipeline(pipeline);
  AddFirst(static_cast<Pipeline &&>(tmp_pipeline));
}
void
Pipeline::AddFirst(Pipeline &&pipeline) {
...
Object *
Pipeline::Fire(Object *arg, bool reverse) {
  Object *result = arg;
  if (!reverse) {
	auto cur_node = first_;
	while (cur_node) {
