...
RbNode *
BasicRbTree::GetRoot() const {
  return h_.node->par;
}
void
BasicRbTree::SetRoot(RbNode *root) {
  h_.node->par = root;
}
...
RbNode *
BasicRbTree::Minimum() const {
  return h_.node->left;
}
RbNode *
BasicRbTree::Maximum() const {
  return h_.node->right;
}
RbNode *
BasicRbTree::Header() const {
  return h_.node;
}
void
BasicRbTree::LowerBound(const void *src, RbNode *&node,
						int (*cmp)(void *, RbNode *, const void *)) const {
  node = RbFindLowerBound(src, h_.node, {(void *)this, cmp});
  if (!node) { node = Header(); }
}
void
BasicRbTree::UpperBound(const void *src, RbNode *&node,
						int (*cmp)(void *, RbNode *, const void *)) const {
  node = RbFindUpperBound(src, h_.node, {(void *)this, cmp});
  if (!node) { node = Header(); }
}
bool
BasicRbTree::Verify(int (*cmp)(void *, RbNode *, RbNode *)) const {
  return RbVerify(Header(), {(void *)this, cmp});
}
bool
BasicRbTree::UniqueAdd(const void *src,
					   int (*cmp)(void *, RbNode *, const void *)) {
  return RbUniqueAdd(src, Header(), {(void *)this, cmp},
					 {(void *)this, &AllocRbNode});
}
bool
BasicRbTree::UniqueAdd(RbNode *hint, const void *src,
					   int (*cmp)(void *, RbNode *, const void *)) {
  return RbUniqueAdd(hint, src, Header(), {(void *)this, cmp},
					 {(void *)this, &AllocRbNode});
}
bool
BasicRbTree::Add(const void *src, int (*cmp)(void *, RbNode *, const void *)) {
  return RbAdd(src, Header(), {(void *)this, cmp},
			   {(void *)this, &AllocRbNode});
}
bool
BasicRbTree::Add(RbNode *hint, const void *src,
				 int (*cmp)(void *, RbNode *, const void *)) {
  return RbAdd(hint, src, Header(), {(void *)this, cmp},
			   {(void *)this, &AllocRbNode});
}
bool
BasicRbTree::Remove(const void *src,
					int (*cmp)(void *, RbNode *, const void *)) {
  return RbRemove(src, Header(), {(void *)this, cmp},
				  {(void *)this, &DropRbNode});
}
bool
BasicRbTree::Remove(RbNode *node) {
  if (node) {
...
unsigned
BasicRbTree::Size() const {
  return node_count_;
}
bool
BasicRbTree::IsEmpty() const {
  return node_count_ == 0 && !h_.node->par && h_.node->left == h_.node
		 && h_.node->right == h_.node;
}
RbNode *
BasicRbTree::Find(const void *src, int (*cmp)(void *, RbNode *, const void *)) {
  RbNode *par_or_target;
  auto ret = RbFindFirst(src, par_or_target, Header(), {(void *)this, cmp});
  return ret == 0 ? par_or_target : Header();
}
BasicRbTree &
BasicRbTree::operator=(const BasicRbTree &t) {
  if (&t != this) {
	if (node_count_) { Clear(); }
	if (t.node_count_) {
	  SetRoot(RbCopy(t.GetRoot(), Header(), {(void *)this, &CloneRbNode},
					 {(void *)this, &DropRbNode}));
	  if (node_count_) {
...
BasicRbTree &
BasicRbTree::operator=(BasicRbTree &&t) noexcept {
  if (&t != this) {
	if (node_count_) { Clear(); }
	SetRoot(t.GetRoot());
	h_.node->left = t.h_.node->left;
...
void
BasicRbTree::DropRbNode(void *arg, RbNode *node) {
...
RbNode *
BasicRbTree::CloneRbNode(void *arg, RbNode *node) {
  auto tree = reinterpret_cast<BasicRbTree *>(arg);
  ++tree->node_count_;
  return tree->ImplCloneRbNode(node);
}
RbNode *
BasicRbTree::AllocRbNode(void *arg, const void *src) {
  auto tree = reinterpret_cast<BasicRbTree *>(arg);
  ++tree->node_count_;
