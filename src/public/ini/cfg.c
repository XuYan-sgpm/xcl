...
{
    if (!str && !((const Region*)arg)->str) { return 0; }
    const Region* region = (const Region*)arg;
    if (!str || !region->str) { return -1; }
    if (strlen(str) != region->len) { return -1; }
 ...
 */
static Parameter* __Config_checkoutParameter(CList* params, Region keyRegion,
                                             Region valueRegion)
{
    CListIter paramIt = List_query(params, &keyRegion, __cmpParamWithRegion);
    if (!List_iterEquals(paramIt, List_end(params)))
 ...
 * in ini parse
 */
static void __Config_iniReadCb(void* usr, Region secRegion, Region keyRegion,
                               Region valueRegion)
{
            ...
            int* valueLen)
{
    ...
    if (secData)
    {
        ...
        Parameter* param =
            __Config_getParameter(secData->parameters, keyRegion);
        if (param)
        {
            int vl = strlen(param->value);
            if (!value) { *valueLen = vl; }
            else
            {
             ...
             const char* value)
{
    ...
    return __Config_checkoutParameter(secData->parameters, keyRegion,
                                      valueRegion)
               ? 0
               : -1;
}
...
static int32_t __Config_streamFlush(void* stream,
                                    int32_t (*writer)(void*, const char*,
                                                      int32_t),
