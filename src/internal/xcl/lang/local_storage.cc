...
void *
LocalStorage::GetObject(unsigned index) const {
  return index >= cap_ ? nullptr : local_objects_[index];
}
void
LocalStorage::PutObject(unsigned int index, void *object) {
  if (index < cap_) {
...
LocalStorage::GenerateId() {
  return atomic_id_.fetch_add(1);
}
unsigned
LocalStorage::GrowStorage(unsigned int index) const {
  if (index < cap_) { return cap_; }
  if (cap_ == 0 && index < initial_cap_) return initial_cap_;
  return Grow(cap_ == 0 ? initial_cap_ : cap_, index);
}
unsigned
LocalStorage::Grow(unsigned int old, unsigned int expect) {
  auto res = expect - old;
