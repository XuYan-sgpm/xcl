...
class Bucket {
 ...
 public:
  void
  PutToBucket(void *key, void *value, unsigned total_elements);
  void *
  GetFromBucket(void *key) const;
 ...
 private:
  class TreeMap {
   ...
   public:
	...
	First() const;
	Node *
	Next(Node *node) const;

   private:
	static int
	Compare(void *, RbNode *node, const void *src);
...
void
Bucket::PutToBucket(void *key, void *value, unsigned total_elements) {
  if (!bucket_context_.is_list) {
	...
	if (list_ptr->size() >= 8 && total_elements >= 64) { TransferToTreeMap(); }
  }
}

void *
Bucket::GetFromBucket(void *key) const {
  if (bucket_context_.is_list) {
	auto *list_ptr = bucket_context_.container.list_ptr;
...
bool
Bucket::TreeMap::Add(void *key, void *value) {
...
unsigned
Bucket::TreeMap::size() const {
  return size_;
}

bool
Bucket::TreeMap::IsEmpty() const {
  return size_ == 0;
}

Bucket::TreeMap::Node *
Bucket::TreeMap::First() const {
  return (Node *)header_->left;
}

Bucket::TreeMap::Node *
Bucket::TreeMap::Next(Node *node) const {
  if (node == header_) { return (Node *)header_->left; }
  if (node->right) {
	node = (Node *)node->right;
	while (node->left) { node = (Node *)node->left; }
	return node;
  }
  Node *par;
  while ((par = (Node *)node->par) != header_) {
	if (node == par->left) { break; }
	node = par;
  }
  return par;
}
...
int
Bucket::TreeMap::Compare(void *, RbNode *node, const void *src) {
  auto diff =
...
bool
Bucket::List::Add(void *key, void *value) {
  auto *node = (Node *)malloc(sizeof(Node));
  if (node) {
	node->key = key;
	node->value = value;
...
void *
Bucket::List::Get(void *key) {
  auto *cur = head_->next;
  while (cur) {
	if (((Node *)cur)->key == key) { return ((Node *)cur)->value; }
	cur = cur->next;
  }
  return nullptr;
}
...
unsigned
Bucket::List::size() const {
  return size_;
}

bool
Bucket::List::IsEmpty() const {
  return size_ == 0;
}

Bucket::List::Node *
Bucket::List::First() const {
  return (Node *)head_->next;
}
...
void
SimpleObjectHashMap::Put(void *key, void *value) {
  if (size_ >= cap_ * 0.75) { Resize(); }
...
void *
SimpleObjectHashMap::Get(void *key) const {
  auto idx = ((long)key) & (cap_ - 1);
  return ((Bucket *)buckets_)[idx].GetFromBucket(key);
}

void
SimpleObjectHashMap::Resize() {
  auto new_cap = cap_ << 1;
...
unsigned
SimpleObjectHashMap::Size() const {
  return size_;
}
}// namespace xcl
