...
public:
  ...
  Object *GetObj() const override;
};

bool NoopHandle::Recycle() { return false; }
Object *NoopHandle::GetObj() const { return nullptr; }
...
AbstractObjectRecycler::AbstractObjectRecycler(
    ...
    : kMaxCapPerThread(max_cap_per_thread), kInterval(interval),
      kMaxDelayedQueuesPerThread(max_delayed_queues_per_thread),
      kDelayedQueueInterval(delayed_queue_interval) {}
AbstractObjectRecycler::AbstractObjectRecycler()
    : kMaxCapPerThread(4096), kInterval(8), kMaxDelayedQueuesPerThread(4),
      kDelayedQueueInterval(8) {}
RecycledHandle *AbstractObjectRecycler::Get() {
  if (kMaxCapPerThread == 0) {
    return nullptr;
  }
...
HandleStack *AbstractObjectRecycler::CreateHandleStack() {
  return new HandleStack(this, pthread_self(), kMaxCapPerThread, kInterval,
                         kMaxDelayedQueuesPerThread, kDelayedQueueInterval);
}
bool AbstractObjectRecycler::Recycle(RecycledHandle *handle) {
  if (handle == noop_handle_) {
    return false;
  }
  auto *h = dynamic_cast<DefaultHandle *>(handle);
  if (!h || h->stack()->recycler() != this) {
    return false;
  }
  h->RecycleToStack();
  return true;
...
void AbstractObjectRecycler::ReleaseLocalHandles() {
  auto stack = reinterpret_cast<HandleStack *>(local_stack_.Get());
  auto map =
...
void HandleStack::Push(DefaultHandle *handle) {
...
void HandleStack::PushNow(DefaultHandle *handle) {
  ...
  if (size >= kMaxCap || DropHandle(handle)) {
    return;
  }
  if (handle_size_ == handle_cap_) {
    auto new_handle_cap = handle_cap_ << 1;
    if (new_handle_cap > kMaxCap) {
      new_handle_cap = kMaxCap;
    }
    auto **new_handles =
...
void HandleStack::PushLater(DefaultHandle *handle, pthread_t thread) {
...
bool HandleStack::DropHandle(DefaultHandle *handle) {
...
HandleOrderQueue *HandleStack::NewOrderQueue(pthread_t t) {
...
HandleStack::HandleStack(AbstractObjectRecycler *recycler, pthread_t t,
                         ...
                         int delayed_queue_interval)
    : recycler_(recycler), thread_(t),
      atomic_available_cap_(max_cap > initial_cap_ ? max_cap : initial_cap_),
      kMaxDelayedQueues(max_delayed_queues), kMaxCap(max_cap),
      kInterval(interval), kDelayedQueueInterval(delayed_queue_interval),
      handle_recycle_count_(interval), cur_queue_(nullptr),
      prev_queue_(nullptr), atomic_head_queue_(nullptr) {
...
void HandleStack::SetHeadQueue(HandleOrderQueue *head_queue) {
...
unsigned HandleStack::IncreaseCapacity(unsigned int expected_cap) {
  auto max_cap = this->kMaxCap;
  auto new_cap = handle_cap_;
  do {
    new_cap <<= 1;
  } while (new_cap < expected_cap && new_cap < max_cap);
  if (new_cap > max_cap) {
    new_cap = max_cap;
  }
...
DefaultHandle *HandleStack::Pop() {
  ...
  handle_size_ = size;
  if (handle->recycle_id() != handle->GetLastRecycleId()) {
    return nullptr;
  }
...
DefaultHandle *HandleStack::NewHandle() { return new DefaultHandle(this); }
bool HandleStack::Scavenge() {
  if (TryScavenge()) {
    return true;
  }
  prev_queue_ = nullptr;
...
AbstractObjectRecycler *HandleStack::recycler() const { return recycler_; }
HandleStack::~HandleStack() {
  for (auto i = 0; i < handle_size_; i++) {
    handles_[i]->Release();
  }
  delete[] handles_;
  handles_ = nullptr;
...
void DefaultHandle::Release() {
  delete obj_;
  obj_ = nullptr;
}
DefaultHandle::DefaultHandle(HandleStack *stack) : stack_(stack) {}
bool DefaultHandle::Recycle() { return stack_->recycler()->Recycle(this); }
bool DefaultHandle::CasLastRecycleId(unsigned expect_id, unsigned update_id) {
  return atomic_last_recycle_id_.compare_exchange_strong(
      expect_id, update_id, std::memory_order_acq_rel);
}
void DefaultHandle::set_object(Object *o) { obj_ = o; }
unsigned DefaultHandle::GetLastRecycleId() const {
  return atomic_last_recycle_id_.load(std::memory_order_acquire);
}
unsigned DefaultHandle::recycle_id() const { return recycle_id_; }
void DefaultHandle::set_recycle_id(unsigned id) { recycle_id_ = id; }
bool DefaultHandle::has_recycled() const { return has_recycled_; }
void DefaultHandle::set_has_recycled(bool v) { has_recycled_ = v; }
HandleStack *DefaultHandle::stack() const { return stack_; }
Object *DefaultHandle::GetObj() const { return obj_; }
void DefaultHandle::set_stack(HandleStack *stack) { stack_ = stack; }
void DefaultHandle::SetLastRecycleId(unsigned int last_recycle_id) {
  atomic_last_recycle_id_.store(last_recycle_id, std::memory_order_release);
}
void DefaultHandle::RecycleToStack() {
...
HandleOrderQueue::Head::Head(std::atomic<unsigned> *stack_atomic_available_cap)
    : stack_atomic_available_cap_(stack_atomic_available_cap), link_(nullptr) {}
void HandleOrderQueue::Head::ReclaimAllAndUnlink() {
...
HandleOrderQueue::Link *HandleOrderQueue::Head::link() const { return link_; }
void HandleOrderQueue::Head::set_link(HandleOrderQueue::Link *link) {
  link_ = link;
}
...
HandleOrderQueue::HandleOrderQueue()
    : thread_(nullptr), head_(nullptr), tail_link_(nullptr),
      kId(AbstractObjectRecycler::atomic_id_generator_.fetch_add(1)),
      kInterval(0), handle_recycle_count_(kInterval) {}
HandleOrderQueue::HandleOrderQueue(HandleStack *stack, pthread_t thread)
    : thread_(thread), head_(&stack->atomic_available_cap_),
      tail_link_(new Link()),
      kId(AbstractObjectRecycler::atomic_id_generator_.fetch_add(1)),
...
HandleOrderQueue *HandleOrderQueue::NewOrderQueue(HandleStack *stack,
                                                  pthread_t thread) {
  if (Head::ReserveForLink(&stack->atomic_available_cap_)) {
    return nullptr;
  }
...
HandleOrderQueue *HandleOrderQueue::GetNextQueue() const { return next_; }
void HandleOrderQueue::SetNextQueue(HandleOrderQueue *queue) { next_ = queue; }
void HandleOrderQueue::ReclaimAllAndUnlink() {
  head_.ReclaimAllAndUnlink();
  next_ = nullptr;
}
void HandleOrderQueue::Add(DefaultHandle *handle) {
  if (!handle->CasLastRecycleId(0, kId))
    return;
  if (handle_recycle_count_ < kInterval) {
    ++handle_recycle_count_;
    handle->Release();
...
bool HandleOrderQueue::HasFinalData() const {
  return tail_link_->read_idx != tail_link_->GetWriteIdx();
}
bool HandleOrderQueue::Transfer(HandleStack *stack) {
  ...
  if (expect_size > stack->handle_cap_) {
    ...
    if (src_end > stack_rest + src_start) {
      src_end = src_start + stack_rest;
    }
  }
...
unsigned HandleOrderQueue::Link::GetWriteIdx() const {
  return atomic_write_idx.load(std::memory_order_acquire);
}
void HandleOrderQueue::Link::SetWriteIdx(unsigned idx) {
  atomic_write_idx.store(idx, std::memory_order_release);
}
pthread_t HandleOrderQueue::thread() const { return thread_; }
void HandleOrderQueue::set_thread(pthread_t thread) { thread_ = thread; }
} // namespace xcl
