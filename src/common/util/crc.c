...
uint16_t crc16_ccitt_false(uint8_t* data, uint16_t length) {
  ...
  uint16_t crc = 0xffff;  // Initial value
  while (length--) {
    crc ^= (uint16_t)(*data++) << 8;  // crc ^= (uint6_t)(*entry)<<8; entry++;
    for (i = 0; i < 8; ++i) {
      if (crc & 0x8000)
        crc = (crc << 1) ^ 0x1021;
      else
...
uint16_t crc16_xmodem(uint8_t* data, uint16_t length) {
  ...
  uint16_t crc = 0;  // Initial value
  while (length--) {
    crc ^= (uint16_t)(*data++) << 8;  // crc ^= (uint16_t)(*entry)<<8; entry++;
    for (i = 0; i < 8; ++i) {
      if (crc & 0x8000)
        crc = (crc << 1) ^ 0x1021;
      else
...
uint32_t crc32_mpeg_2(uint8_t* data, uint16_t length) {
  ...
  uint32_t crc = 0xffffffff;  // Initial value
  while (length--) {
    crc ^= (uint32_t)(*data++) << 24;  // crc ^=(uint32_t)(*entry)<<24; entry++;
    for (i = 0; i < 8; ++i) {
      if (crc & 0x80000000)
...
uint32_t crc32Init() { return 0xffffffff; }

uint32_t crc32Update(uint32_t val, uint8_t* data, uint64_t length) {
  while (length--) {
    val ^= (uint32_t)(*data++) << 24;  // crc ^=(uint32_t)(*entry)<<24; entry++;
    for (int i = 0; i < 8; ++i) {
      if (val & 0x80000000)
...
uint32_t crc32Final(uint32_t val) { return ~val; }
