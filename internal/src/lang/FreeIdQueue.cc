...
bool
FreeIdQueue::isEmpty() const {
  xcl::Locker locker(lock_);
  return size_ == 0;
}
bool
FreeIdQueue::poll(int64_t* id) {
  xcl::Locker locker(lock_);
  if (size_ == 0) {
    return false;
  }
...
offerFreeId(int64_t id) {
  return __idQueue.offer(id);
}
bool
hasFreeId() {
  return __idQueue.isEmpty();
}
bool
pollFreeId(int64_t* id) {
  return __idQueue.poll(id);
}
