...
class ThreadImpl final {
 ...
 public:
  ThreadImpl &
  operator=(const ThreadImpl &) = delete;
  bool
  operator==(const ThreadImpl &thread_impl) const;

 public:
  bool
  PushRunnable(CallPair call_pair);
  bool
  IsThreadRunning();
  bool
  JoinThread();
 ...
 private:
  ...
  CreateLocalKey();
  static void
  SetLocalThread(ThreadImpl *thread_impl);
};
}// namespace xcl::thread_impl
