...
class SizeClasses {
protected:
  static unsigned Log2(int i);
  static bool IsPow2(unsigned i);
...
private:
  unsigned CalSizeClasses();
  unsigned CalSizeClass(unsigned idx, unsigned log_2_group,
                        unsigned log_2_delta, unsigned n_delta);
  void CalIdx2SizeTab();
  void CalSize2IdxTab();

public:
  unsigned SizeIdx2Size(unsigned size_idx) const;
  unsigned PageIdx2Size(unsigned page_idx) const;
  unsigned Size2SizeIdx(unsigned size) const;
  unsigned Page2PageIdx(unsigned pages) const;
  unsigned Normalize(unsigned size) const;
  unsigned GetSizes() const;
  unsigned GetChunkSize() const;
  unsigned GetPages() const;

private:
  static SizeClasses *size_obj_ptr_;

public:
  static SizeClasses &Instance();

public:
  SizeClasses &operator=(const SizeClasses &) = delete;
