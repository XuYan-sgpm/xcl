...
public:
  ...
  bool CasLastRecycleId(unsigned expect_id, unsigned update_id);
  void SetLastRecycleId(unsigned last_recycle_id);
  void set_object(Object *o);
  unsigned GetLastRecycleId() const;
  unsigned recycle_id() const;
  void set_recycle_id(unsigned id);
  bool has_recycled() const;
  void set_has_recycled(bool v);
  HandleStack *stack() const;
  void set_stack(HandleStack *stack);
...
public:
  struct Link {
    ...
    unsigned GetWriteIdx() const;
    void SetWriteIdx(unsigned idx);
  };
...
public:
  static HandleOrderQueue *NewOrderQueue(HandleStack *stack, pthread_t thread);
  HandleOrderQueue *GetNextQueue() const;
  void SetNextQueue(HandleOrderQueue *queue);
  void ReclaimAllAndUnlink();
  void Add(DefaultHandle *handle);
  bool HasFinalData() const;
  bool Transfer(HandleStack *stack);
...
public:
  void SetHeadQueue(HandleOrderQueue *head_queue);
  unsigned IncreaseCapacity(unsigned expected_cap);
  DefaultHandle *Pop();
  void Push(DefaultHandle *handle);
  bool DropHandle(DefaultHandle *handle);
  DefaultHandle *NewHandle();
  AbstractObjectRecycler *recycler() const;

private:
  bool Scavenge();
  bool TryScavenge();
  void PushNow(DefaultHandle *handle);
  void PushLater(DefaultHandle *handle, pthread_t thread);
  HandleOrderQueue *NewOrderQueue(pthread_t t);
};
} // namespace xcl
