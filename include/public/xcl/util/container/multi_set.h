...
class MultiSet final {
 ...
 private:
  class TreeContext final : public BasicRbTree {
   ...
   protected:
	RbNode *
	ImplAllocRbNode(const void *src) override;
	RbNode *
	ImplCloneRbNode(const RbNode *node) override;
	void
	ImplReleaseRbNode(RbNode *node) override;
	void
	ImplDeallocRbNode(RbNode *node) override;
 ...
 private:
  static int
  CompareValue(void *args, RbNode *node, const void *src);
  static int
  CompareNode(void *args, RbNode *node1, RbNode *node2);
 ...
 public:
  ...
  Clear();
  E &
  Find(const E &e) const;
};

template<typename E>
RbNode *
MultiSet<E>::TreeContext::ImplAllocRbNode(const void *src) {
...
template<typename E>
RbNode *
MultiSet<E>::TreeContext::ImplCloneRbNode(const RbNode *node) {
  return ImplAllocRbNode(&((const Node *)node)->value);
}
template<typename E>
void
MultiSet<E>::TreeContext::ImplReleaseRbNode(RbNode *node) {
  DefObjInterface<E>::instance().destroy(&((Node *)node)->value, 0, nullptr);
}
template<typename E>
void
MultiSet<E>::TreeContext::ImplDeallocRbNode(RbNode *node) {
...
template<typename E>
bool
MultiSet<E>::Verify() const {
  return context_->verify(&CompareNode);
}
template<typename E>
bool
MultiSet<E>::Add(const E &e) {
  return context_->add(&e, &CompareValue);
}
template<typename E>
unsigned
MultiSet<E>::Size() const {
  return context_->size();
}
template<typename E>
int
MultiSet<E>::CompareValue(void *args, RbNode *node, const void *src) {
  return ((TreeContext *)args)
	  ->comparator_(((Node *)node)->value, *(const E *)src);
}
template<typename E>
int
MultiSet<E>::CompareNode(void *args, RbNode *node1, RbNode *node2) {
  return ((TreeContext *)args)
...
template<typename E>
E &
MultiSet<E>::Find(const E &e) const {
